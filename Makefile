...
it_preparation:
	...
	rsp rasterize --config config.toml --zoom 18 --geojson it/lyon_roofprint.json --web_ui it/cover it/labels_osm
...
it_train:
	rsp train --config config.toml --workers 2 --epochs 5 --batch_size 4 --dataset it it/pth
	rsp train --config config.toml --workers 2 --batch_size 4 --resume --checkpoint it/pth/checkpoint-00005-of-00005.pth --epochs 10 --dataset it it/pth
